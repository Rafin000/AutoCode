...
function resolveRepo(config: Config, repoName: string | undefined): RepoConfig {
  ...
  if (!name) {
    ...
    console.error("Run `autocode repo add <name> <path>` first.");
    process.exit(1);
  }
...
export async function featureCreateCommand(
  ...
  try {
    ...
    console.log(`  autocode feature plan ${feature.id}`);
    console.log(`  autocode feature implement ${feature.id}`);
  } catch (err) {
    updateFeature(feature.id, {
      status: "failed",
...
export async function featureImplementCommand(
  ...
  if (feature.status !== "plan_ready" && feature.status !== "pending") {
    ...
    console.error("  Run `autocode feature plan " + id + "` to see its current plan.");
    process.exit(1);
  }
...
export async function featurePlanCommand(id: string): Promise<void> {
  ...
  if (!feature.implementation_plan) {
    ...
    console.error("  Run `autocode feature create` to start with a plan,");
    console.error("  or `autocode feature status ${id}` to see its current state.");
    process.exit(1);
  }
...
async function runImplementation(
  ...
  console.log();
  console.log("Next steps:");
  console.log(`  autocode feature status ${feature.id}`);
  console.log(`  autocode feature approve ${feature.id}    (after merging the PR)`);
}
...
function buildPrBody(
  ...
  lines.push("---");
  lines.push(`_Generated by autocode. Feature ID: \`${featureId}\`_`);
  return lines.join("\n");
}
...
export async function featureListCommand(opts: { repo?: string }): Promise<void> {
  ...
  if (features.length === 0) {
    ...
    console.log('Create one: autocode feature create -t "Title" -d "Description"');
    return;
  }
...
export async function featureApproveCommand(
  ...
  try {
    ...
    console.warn("  (The feature is still approved — run `autocode sync` manually to recover.)");
  }
}
